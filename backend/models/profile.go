...
func GetProfiles(db *sql.DB) ([]Profile, error) {
	fmt.Println("Fetching profiles from the database...") // Log to indicate function is called
	...
	for rows.Next() {
		...
		fmt.Printf("Fetched profile: %v", p)

		profiles = append(profiles, p)
	}

	// Log the total number of profiles fetched
	fmt.Printf("Total profiles fetched: %d", len(profiles))
